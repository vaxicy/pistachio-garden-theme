...
.tea.small p{font-size:13px;margin-top:18px;color:#F1F3E0}
.tea.small:after{content:'';position:absolute;left:0;right:0;bottom:0;height:14px;background:#D2DCB6}
.tea.wide{width:1400px;height:560px;background:#D2DCB6;color:#354332}
...
.tea .window{position:absolute;left:550px;top:83px;transform:scale(.62);transform-origin:top left;border:2px solid #778873;border-radius:14px;overflow:hidden;color:var(--ink)}
...
.google{margin-bottom:20px;color:#778873}
.search{height:42px}
.tea .window .content{padding-top:85px}
"""

css += """
.poster{width:1400px;height:560px;background:var(--milk);position:relative;overflow:hidden;text-align:center;border-top:8px solid #778873}
.poster h1{font:48px Georgia,serif;margin:23px 0 7px}
.poster p{font-size:17px;margin:0}
.poster .preview{position:absolute;left:284px;top:145px;transform:scale(.65);transform-origin:top left;border:2px solid #778873;border-radius:16px;overflow:hidden;text-align:left}
...
"""
css += """
.poster{background:#D2DCB6;text-align:left;border-top:0}
.poster h1{position:absolute;left:52px;top:78px;width:390px;font-size:50px;line-height:1.1;margin:0}
.poster>p{position:absolute;left:52px;top:260px;width:370px;line-height:1.6}
.poster .preview{left:490px;top:92px;transform:scale(.67);border-color:#778873}
...
colors=[('Garden Green','frame','Window frame','#FFFFFF'),('Soft Leaf','leaf','Botanical accent','#354332'),('Pistachio Green','toolbar','Toolbar & active tab','#354332'),('Cream Green','ntp_background','New tab background','#354332')]
cards=''.join(f'<div class="card" style="background:{color(k)};color:{fg};border:1px solid #CED8D7"><strong>{name}</strong><span>{color(k)} · {role}</span></div>' for name,k,role,fg in colors)
intro='<div class="intro"><div class="kicker">A LITTLE GREEN, A LITTLE CALM</div><h1>Pistachio Garden Theme</h1><p>Four colors. One refreshing space.</p><div class="cards">'+cards+'</div><p style="font-size:16px;margin-top:30px">Solid colors · Minimal design · No wallpaper · No permissions required</p></div>'
